...
const YELLOW = "#facc15";   // ok
const RED    = "#f87171";   // bad
const BLUE   = "#60a5fa";   // neutral accent (charts, info)
const PURPLE = "#c084fc";   // multi-kill highlight
const DIM    = "#3f3f46";   // zero/empty values
...
// ── Map name cleaner: removes de_ / cs_ prefix and capitalises ───────────────
function cleanMap(name) {
  if (!name || name === "Unknown") return name;
  return name
    .replace(/^(de_|cs_|ar_|gg_)/i, "")
...
function buildRadar(lt, history) {
  ...
  return [
    { stat: "Aim",      value: Math.min(100, Math.round(avgKd * 55)) },
    { stat: "HS%",      value: Math.min(100, Math.round(avgHs * 1.5)) },
    { stat: "Win Rate", value: Math.min(100, Math.round(wr * 1.4)) },
    { stat: "ADR",      value: Math.min(100, Math.round(avgAdr)) },
    { stat: "Clutch",   value: Math.min(100, clutchTot * 5) },
...
export default function App() {
  ...
  async function search() {
    ...
    try {
      ...
      setStats(s);
      // Clean map names in history
      setHistory(h.map(m => ({ ...m, map: cleanMap(m.map) })));
      setPatterns(pt);
    } catch (e) {
      setError(e.message || "Something went wrong. Is your backend running on port 8000?");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-btn {
          ...
          color: #52525b; border: none; background: none;
      ...
      <aside style={{
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {error && (
            <div style={{ marginTop: 8, fontSize: 11, color: RED, lineHeight: 1.5, padding: "8px 10px", background: "rgba(248,113,113,0.08)", borderRadius: 6 }}>
              ⚠ {error}
            </div>
          )}
        </div>
          ...
          <div style={{ marginTop: "auto", paddingTop: 14, paddingBottom: 16, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ position: "relative", flexShrink: 0 }}>
                <img
                  src={player.avatar || `https://i.pravatar.cc/40?u=${player.nickname}`}
                  alt=""
                  style={{ width: 36, height: 36, borderRadius: "50%", display: "block", border: `2px solid ${LEVEL_COLORS[player.level] || "#555"}` }}
                />
                <div style={{ position: "absolute", bottom: -3, right: -3, background: LEVEL_COLORS[player.level] || "#555", color: "#000", borderRadius: 4, padding: "1px 4px", fontSize: 9, fontWeight: 800 }}>
                  {player.level}
                </div>
              </div>
          ...
          <div className="animated">
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <img
                  src={player.avatar || `https://i.pravatar.cc/56?u=${player.nickname}`}
                  alt=""
                  style={{ width: 52, height: 52, borderRadius: "50%", border: `3px solid ${LEVEL_COLORS[player.level] || "#555"}` }}
                />
                <div>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 12 }}>
                    ...
                    <span style={{ color: "#27272a" }}>|</span>
                    <a href={player.faceit_url} target="_blank" rel="noreferrer" style={{ color: GREEN, textDecoration: "none", fontSize: 11 }}>
                      Faceit Profile ↗
                    </a>
                  </div>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 280px", gap: 14 }}>
                <Card>
                  ...
                  <ResponsiveContainer width="100%" height={185}>
                    <AreaChart data={history}>
                      ...
                      <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} interval={3} />
                      <YAxis tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} domain={[0, "auto"]} />
                      <Tooltip content={<Tip />} />
                ...
                <Card style={{ gridColumn: "1 / 3" }}>
                  ...
                  <ResponsiveContainer width="100%" height={130}>
                    <BarChart data={history} barSize={12}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1c1c1e" />
                      <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} interval={3} />
                      <YAxis tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} />
                      <Tooltip content={<Tip />} />
                      <Bar dataKey="kills" name="Kills" radius={[3, 3, 0, 0]}>
                        {history.map((m, i) => <Cell key={i} fill={m.result === "W" ? GREEN : RED} opacity={0.85} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                ...
                <Card>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <Badge label="Triples"    value={lt.triple_kills} color={YELLOW}  />
                    <Badge label="Quadros"    value={lt.quadro_kills} color={PURPLE}  />
                    <Badge label="Pentas"     value={lt.penta_kills}  color={GREEN}   />
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <ScatterChart>
                      ...
                      <Tooltip cursor={{ strokeDasharray: "3 3" }} content={({ active, payload }) => {
                        ...
                        return (
                          <div style={{ background: "#18181b", border: "1px solid #27272a", borderRadius: 8, padding: "9px 13px", fontSize: 12 }}>
                            <div style={{ color: d?.result === "W" ? GREEN : RED, fontWeight: 700, marginBottom: 4 }}>{d?.result} · {d?.map}</div>
                ...
                <Card>
                  ...
                  <ResponsiveContainer width="100%" height={170}>
                    <BarChart data={history} barSize={12}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1c1c1e" />
                      <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} interval={3} />
                      <YAxis tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} unit="%" />
                      <Tooltip content={<Tip />} />
                      <Bar dataKey="hs_pct" name="HS%" radius={[3, 3, 0, 0]}>
                        {history.map((m, i) => <Cell key={i} fill={hsColor(m.hs_pct)} opacity={0.85} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </Card>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <Card>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={history} barSize={10}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1c1c1e" />
                      <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} interval={3} />
                      <YAxis tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} allowDecimals={false} />
                      <Tooltip content={<Tip />} />
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={history} barSize={12}>
                      ...
                      <Bar dataKey="first_kills" name="First Kills" radius={[3, 3, 0, 0]}>
                        {history.map((m, i) => <Cell key={i} fill={m.first_kills >= 4 ? GREEN : m.first_kills >= 2 ? YELLOW : BLUE} opacity={0.85} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </Card>
                ...
                <Card>
                  ...
                  <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 10, padding: "14px 16px" }}>
                    ...
                    {(() => {
                      ...
                      if (er <  10 && ct >= 5) return <p style={{ fontSize: 13, color: "#a1a1aa", lineHeight: 1.6 }}>🧠 <strong style={{ color: "#fff" }}>Clutch Player</strong> — You excel in late-round situations. Consider being more aggressive early on CT-side.</p>;
                      if (er >= 15 && ct >= 5) return <p style={{ fontSize: 13, color: "#a1a1aa", lineHeight: 1.6 }}>⭐ <strong style={{ color: "#fff" }}>Star Player</strong> — High entry rate AND strong clutch numbers. You're a rare dual-threat player.</p>;
                      return                          <p style={{ fontSize: 13, color: "#a1a1aa", lineHeight: 1.6 }}>📊 <strong style={{ color: "#fff" }}>Balanced Role</strong> — Well-rounded play. More matches needed to confirm a role specialization.</p>;
                    })()}
                  </div>
                </Card>
                ...
                <Card>
                  ...
                  <ResponsiveContainer width="100%" height={155}>
                    <BarChart data={history.filter(m => m.flash_count > 0)} barSize={12}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1c1c1e" />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <Card style={{ gridColumn: "1 / -1" }}>
                  ...
                  <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
                    <Badge label="Triple Kills" value={totalTriples} color={YELLOW} />
                    <Badge label="Quadro Kills" value={totalQuadros} color={PURPLE} />
                    <Badge label="Penta Kills"  value={totalPentas}  color={GREEN}  />
                  ...
                  <ResponsiveContainer width="100%" height={150}>
                    <BarChart data={history} barSize={8}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1c1c1e" />
                      <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} interval={2} />
                      <YAxis tick={{ fontSize: 10, fill: "#444" }} tickLine={false} axisLine={false} allowDecimals={false} />
                      <Tooltip content={<Tip />} />
                      <Bar dataKey="triple" name="Triple" stackId="mk" fill={YELLOW} opacity={0.85} />
                      <Bar dataKey="quadro" name="Quadro" stackId="mk" fill={PURPLE} opacity={0.85} />
                      <Bar dataKey="penta"  name="Penta"  stackId="mk" fill={GREEN}  opacity={0.85} radius={[3, 3, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                  <div style={{ display: "flex", gap: 14, marginTop: 8 }}>
                    <span style={{ fontSize: 11, color: YELLOW }}>■ Triple</span>
                    <span style={{ fontSize: 11, color: PURPLE }}>■ Quadro</span>
                    <span style={{ fontSize: 11, color: GREEN }}>■ Penta</span>
                  </div>
                </Card>
                ...
                <Card>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 4 }}>
                    {[
                      { label: "Triple Kills", value: lt.triple_kills, color: YELLOW, pct: Math.min(100, parseInt(lt.triple_kills) / 5) },
                      { label: "Quadro Kills", value: lt.quadro_kills, color: PURPLE, pct: Math.min(100, parseInt(lt.quadro_kills) / 2) },
                      { label: "Penta Kills",  value: lt.penta_kills,  color: GREEN,  pct: Math.min(100, parseInt(lt.penta_kills) * 5) },
                    ].map((r, i) => (
                      <div key={i}>
                ...
                <Card>
                  ...
                  <div style={{ overflowY: "auto", maxHeight: 230 }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                      ...
                      <tbody>
                        {history.filter(m => m.triple + m.quadro + m.penta > 0).map((m, i) => (
                        ...
                        {history.filter(m => m.triple + m.quadro + m.penta > 0).length === 0 && (
                          <tr><td colSpan={5} style={{ padding: "20px 10px", color: "#52525b", fontSize: 13 }}>No multi-kill matches in recent history</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
            ...
            {tab === "maps" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <Card>
                  <Title sub={`${mapData.reduce((a, m) => a + m.matches, 0).toLocaleString()} total career matches`}>
                    All-Time Win Rate by Map
                  </Title>
                ...
                <Card style={{ gridColumn: "1 / -1" }}>
                  ...
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 700 }}>
                      <thead>
                        <tr>
                          {["Map", "Matches", "Wins", "Win Rate", "K/D", "K/R", "Avg K", "Avg D", "HS%", "3K", "4K", "5K"].map(h => (
              ...
              <Card>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 900 }}>
                    <thead>
                      <tr>
                        {["Result","Map","Score","K","D","A","K/D","ADR","HS%","3K","4K","5K","1v1","1v2","Entry","MVPs","Date"].map(h => (
                      ...
                      {history.map((m, i) => (
                        <tr key={i} className="t-row">
                          <td style={{ padding: "10px 11px" }}>
                            <span style={{
                              background: m.result === "W" ? "rgba(74,222,128,0.12)" : m.result === "L" ? "rgba(248,113,113,0.12)" : "rgba(255,255,255,0.04)",
                              color: m.result === "W" ? GREEN : m.result === "L" ? RED : "#555",
                              borderRadius: 5, padding: "2px 8px", fontWeight: 800, fontSize: 11
                            }}>{m.result}</span>
                          </td>
